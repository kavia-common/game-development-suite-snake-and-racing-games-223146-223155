 ...
 * Props:
 * - username: string
 * - onSelectGame: (gameKey: 'Snake' | 'Racing' | 'Bubble Shooter' | 'Building Blocks') => void
 * - onLogout: () => void
 */
// PUBLIC_INTERFACE
      ...
      <main className="app-content" role="main">
        <div className="game-section" style={{ maxWidth: 720, margin: "36px auto" }}>
          ...
          <div style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", marginTop: 18 }}>
            ...
            <button
              className="btn"
              onClick={() => onSelectGame("Building Blocks")}
              aria-label="Play Building Blocks"
              style={{ padding: "18px 12px", justifySelf: "stretch" }}
            >
              🏗️ Play Building Blocks
            </button>
          </div>
        </div>
      </main>
